...
export class AuthEffects {
  ...
  battalionRegisterStart = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        return this.db
          .doc(`battalionCodeTracker/battalionCode`)
          .valueChanges()
          .pipe(
            take(1),
            map((callBackData: any) => {
              ...
              if (data.registrationType === 'battalion') {
                if (!isTaken) {
                  ...
                  return AuthActions.authenticateFail({
                    error: 'System Error - Battalion Code already taken',
                  });
                }
              } else {
                if (isTaken) {
                  ...
                  return AuthActions.authenticateFail({
                    error: 'Battalion Code not found',
                  });
                }
              }
            })
    ...
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        return from(
          ...
          map((userCredential) => {
            if (data.registrationType === 'battalion') {
              return AuthActions.battalionRegisterSuccess({
                ...data,
                ...userCredential,
              });
            } else {
              return AuthActions.battalionInstructorRegisterSuccess({
                ...data,
                ...userCredential,
              });
            }
          ...
          tap(() => {
            firebase
              .analytics()
              .logEvent('sign_up', { userType: 'instructor' });
          }),
          catchError((err) => {
            return handleError(err.code, err.message);
          })
  ...
  battalionRegisterSuccess = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        return forkJoin(
          from(
            this.db
              .collection('battalionCodeTracker')
              .doc('battalionCode')
              .set(
                {
          ...
          tap(() => {
            firebase
              .analytics()
              .logEvent('sign_up', { userType: 'battalion_registration' });
          })
        );
      })
    )
  ...
  battalionInstructorRegisterSuccess = createEffect(() =>
    this.actions$.pipe(
    ...
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        return from(
          ...
          map((userCredential) => {
            return AuthActions.cadetSignupSuccess({
              ...data,
              ...userCredential,
            });
          }),
  ...
  cadetSignupSuccess = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        ...
        return forkJoin(
          ...
          from(
            this.db
              .doc(
                `battalions/${data.battalionCode}/cadetsProgress/${data.battalionCode}`
              )
              .set({ [data.user.uid]: { ...appUserData } }, { merge: true })
          )
        ).pipe(
          map(() => {
            return AuthActions.loginStart({
  ...
  uploadProfileImage = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        ...
        return forkJoin(from(image.snapshotChanges()), from(image)).pipe(
          tap(() => {
            if (data[1].user.photoUrl) {
              this.storage.storage
                .refFromURL(`${data[1].user.photoUrl}`)
                .delete();
            }
          }),
          switchMap(() => {
            return from(ref.getDownloadURL()).pipe(
              tap((url) => {
  ...
  updateUserInfo = createEffect(() =>
    this.actions$.pipe(
      ...
      tap((data) => {
        ...
        const dataFromLocalStorageInfo = JSON.parse(
          localStorage.getItem('userData')
        );
        dataFromLocalStorageInfo.displayName =
          data[0].firstName + ' ' + data[0].lastName;
        dataFromLocalStorageInfo.firstName = data[0].firstName;
        dataFromLocalStorageInfo.lastName = data[0].lastName;
        localStorage.setItem(
          'userData',
          JSON.stringify(dataFromLocalStorageInfo)
        );
      }),
      switchMap((data: any) => {
        return of(
    ...
    this.actions$.pipe(
      ...
      switchMap((data: any) => {
        ...
        return this.afAuth.auth.currentUser
          ...
          .then(() => {
            return this.afAuth.auth.currentUser.updatePassword(
              data[0].newPassword
            );
          })
          .then(() => {
              ...
              .pipe(
                map((rdata: any) => {
                  ...
                  if (battalionCodes.includes(newBattalionCode)) {
                    return AuthActions.updateCodeExist({
                      newBattalionCode,
                      oldBattalionCode,
                      cadetUid,
                    });
  ...
  updateBattalionCodeExist = createEffect(() =>
    this.actions$.pipe(
  ...
  updateBattalionCodeRetrieved = createEffect(() =>
    this.actions$.pipe(
        ...
        return combineLatest(
          ...
          from(
            this.db
              ...
              .collection('cadetsProgress')
              .doc(newBattalionCode)
              .set({ [cadetUid]: cadetsProgress }, { merge: true })
          )
        ).pipe(
          tap(() => {
            combineLatest(
